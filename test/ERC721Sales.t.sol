...
contract ERC721SalesTest is Test, ERC721Holder {
  ...
  function _balc(
    ...
    address tokenCtrt,
    string memory nameTheOther
  ) private view returns (uint256 uTok, uint256 uNft, uint256 cTok, uint256 cNft) {
  ...
  function testSellNFTviaERC20() external {
    ...
    amount = 1000;
    boo = usdt.transfer(salesAddr, amount * 1e6);
    assert(boo);
