...
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol"; //includes IERC20, IERC20Permit, safeTransfer, safeTransferFrom, safeApprove, safeIncreaseAllowance, safeDecreaseAllowance
...
contract ArrayOfStructs {
  ...
  function getBox2(
    uint256 id
  ) public view returns (Box memory box) {
    box = boxes[id];
  }
  ...
  function getBoxes2(
    uint256 min,
    uint256 max
  ) public view returns (Box[] memory out) {
    uint256 arrlength = max - min + 1;
